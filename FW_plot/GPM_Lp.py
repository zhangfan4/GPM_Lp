...
def GPM(A, x, y, alpha, p, radius):
    ...
    stopping_tol = 1e-10     # the tolerance of stopping criteria
    ...
    while True:
        ...
        if abs(LA.norm(x, p) ** p - radius) <= tol:
            ...
            if abs(LA.norm(z, p) ** p - radius) <= tol or LA.norm(z, p) ** p <= radius:  # Will this situation happen?
                # print('outer-outer')
                x = z
            ...
            if (LA.norm(z, p) ** p - radius) <= tol:
                ...
                print('inner-outer')
                # print('11'*30)
                # print(LA.norm(x, p) ** p - radius)
                # print(LA.norm(z, p) ** p - radius)
                # print('11' * 30)
                dim = len(z)
                x_ini = x  # the initial points for the algorithm
                
                '''Three ways to initialize epsilon'''
                # 1. Generate epsilon according to x.
                epsilon = 0.9 * (1. / dim * (radius - LA.norm(x, p) ** p)) ** (1. / p) * np.ones(dim)  # ensure that the point is feasible.

                # 2. Root finding for epsilon on the boundary
                # low = 0
                # up = (radius / dim) ** (1./p)
                # tol_bi = 1e-10
                # print('LA.norm(x, p) ** p - radius:', LA.norm(x, p) ** p - radius)
                #
                # def fun_eval(eps):
                #     """
                #         Function evaluation
                ...
                #     """
                #     return LA.norm(abs(x)+eps, p) ** p - radius
                # sol = root_scalar(fun_eval, method='toms748', bracket=[low, up], xtol=tol_bi)
                # epsi = sol.root
                # print('epsi:', epsi)
                # epsilon = 0.9 * epsi * np.ones(dim)

                # 3. Randomly generating epsilon.
                # radius_rest = radius - LA.norm(x_ini, p) ** p
                # rand_num = np.random.uniform(0, 1, dim)
                # abs_norm = LA.norm(rand_num, ord=1)
                # epsilon = 0.9 * (rand_num * radius_rest / abs_norm)**p  # ensure that the point is feasible.
                # print('initial residual:', radius_rest)
                ...
                #     x = WeightedL1Projection(z, w, radius_L1)
                # else:
                print('Lp ball projection')
                # print(epsilon)
                x, save_flag = Lp_proj.WeightLpBallProjection(dim, x_ini, z, p, radius, epsilon)
                if save_flag is True:
                    np.save('failed2/x0.npy', x_ini)
                    np.save('failed2/y.npy', z)
                    np.save('failed2/radius.npy', radius)
                    exit()
