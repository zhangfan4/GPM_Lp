...
# %% Subproblem solve: exact projection onto the weighted simplex 
    ...
    if LA.norm(y, p) ** p <= radius: #  Determine the current ball whether it falls into the lp-ball.
        ...
        while True:
            ...
            if count == 1:
                x = abs(x)
            #%% Step 3 of algorithm1: Reweighting: Compute the weights 
            weights = p * (np.abs(x) + epsilon) ** (p-1)
            weights_seq += [weights]
            
            #%% Step 4 of algorithm1: Subproblem solving

            # print(radius - LA.norm(np.abs(x), p) ** p - LA.norm(epsilon, p) ** p)
            ...
            condition_left = LA.norm(local_reduction, ord=2) * LA.norm(sign_weight, ord=2) ** Tau
            condition_right = M
